#!/usr/bin/env bash
#
# Generate browsable API documentation for the jsonschema library from ddoc
# comments.
#
# Strategy:
...
# The base package plus the vibe subpackage adapter — the whole public surface.
SRC_PATHS=(source/jsonschema vibe/source/jsonschema)
...
gen_with_adrdox() {
  ...
  rm -rf "$OUTDIR"
  mkdir -p "$OUTDIR"
  # -i : generate a per-package index ; -o : output directory
  "$ADRDOX_BIN" -i -o "$OUTDIR" "${SRC_PATHS[@]}"
